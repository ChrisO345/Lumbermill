package com.chrisoliver345.lumbermill.client.gui.screen.inventory;
...
@OnlyIn(Dist.CLIENT)
public class LumbermillScreen extends ContainerScreen<LumbermillContainer>
{
    private static final ResourceLocation BACKGROUND_TEXTURE = new ResourceLocation(Reference.MOD_ID, "textures/gui/container/lumbermill.png");
    private float sliderProgress;
    private boolean clickOnScrollBar;
    private int recipeIndexOffset;
    private boolean hasItemsInInputSlot;

    public LumbermillScreen(LumbermillContainer container, PlayerInventory playerInventory, ITextComponent titleIn)
    {
    ...
    {
        List<LumbermillingRecipe> list = this.container.getRecipeList();
        for(int i = this.recipeIndexOffset; i < indexOffset && i < this.container.getRecipeListSize(); ++i)
        {
            int index = i - this.recipeIndexOffset;
            int recipeOffsetX = offsetX + index % 4 * 16;
